...
<!-- hyperdx-release-notes version=2.40.0 inputs=f2f078d054f0 -->
...
tidier bar and a toolbar that stays put as a long query wraps, and the `hdx
chart` command picks up the compact duration axis labels the web app already
uses, so a tick reads "13m" instead of "13.33min". The bundled OTel collector
also ships with the StatsD receiver compiled in, ready for a custom pipeline to
ingest StatsD and DogStatsD metrics directly.
